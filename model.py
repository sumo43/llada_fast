...
class ModelArgs:
    ...
    def from_name(cls, name: str):
        if name in transformer_configs:
            config_dict = transformer_configs[name].copy()
        ...
        config_dict = transformer_configs[config[0]].copy()
        config_dict["model_name"] = name
        return cls(**config_dict)
...
transformer_configs = {
    ...
    "LLaDA-8B-Instruct": dict(block_size=131072, n_layer=32, n_head=32, n_local_heads=32, dim=4096, intermediate_size=12288, vocab_size=126464, rope_base=500000,
...
class Transformer(nn.Module):
    def __init__(self, config: ModelArgs) -> None:
        super().__init__()
        self.config = config
        self._model_name = getattr(config, "model_name", "")
    ...
    def forward(self, mask: BlockMask, idx: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        # Track if this is a LLaDA model for later components to reference
        is_llada = "LLaDA" in getattr(self, "_model_name", "")
...
class Attention(nn.Module):
    ...
    def forward(self, x: Tensor, freqs_cis: Optional[Tensor] = None, mask: Optional[BlockMask] = None, input_pos: Optional[Tensor] = None) -> Tensor:
        ...
        # Handle missing freqs_cis for LLaDA model
        ...
        """
        # Handle missing mask for direct attention computation in LLaDA model
        if mask is None:
            # For LLaDA when doing direct computation, use a simpler attention implementation
            # that doesn't require a mask
            if self.n_head != self.n_local_heads:
                # Handle GQA if needed - repeat k and v for each query head
                k = k.repeat_interleave(self.n_head // self.n_local_heads, dim=1)
                v = v.repeat_interleave(self.n_head // self.n_local_heads, dim=1)
            
            # Do a standard attention calculation without masking
            attn_weights = torch.matmul(q, k.transpose(-2, -1)) / math.sqrt(self.head_dim)
            attn_weights = F.softmax(attn_weights, dim=-1)
            y = torch.matmul(attn_weights, v)
        else:
            # Standard path using flex_attention
            # Use flex_attention with the appropriate masking
            # The mask itself will be configured for causal/non-causal based on the mask_mod function
        """
        y = flex_attention(q, k, v, block_mask=None, enable_gqa=(self.n_head != self.n_local_heads))
...
class FeedForward(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        # For LLaDA and other models using SwiGLU activation
